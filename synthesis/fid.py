...
def calculate_inception_stats(
    ...
    # make_patches=((128, 128), (50, 50))
    make_patches=((256, 256), (256, 256))
):
    # Rank 0 goes first.
    if dist.get_rank() != 0:
    ...
    for images, _labels in tqdm.tqdm(data_loader, unit='batch', disable=(dist.get_rank() != 0)):
        ...
        if images.shape[-2:] != (512, 512):
            images = torch.nn.functional.interpolate(images, size=(512, 512), mode='bilinear', antialias=True)
        if make_patches is not None:
            patches_size, patches_stride = make_patches
            images = (torch.nn.functional.unfold(images.float(), kernel_size=patches_size, stride=patches_stride)
                      .permute(0, 2, 1).reshape(-1, images.shape[1], *patches_size))
    ...
    mu /= total_size
    sigma -= mu.ger(mu) * total_size
    sigma /= total_size - 1
    print("Inception stats computed for", total_size, "patches")
...
def calculate_fid_from_inception_stats(mu, sigma, mu_ref, sigma_ref):
    ...
    fid = m + np.trace(sigma + sigma_ref - np.real(s) * 2)
...
@click.option('--batch',                help='Maximum batch size', metavar='INT',                   type=click.IntRange(min=1), default=2, show_default=True)
...
def just_calc(image_path, ref_path, num_expected, seed, batch, make_patches=((128, 128), (50, 50))):
    """Calculate FID for a given set of images."""

    with dnnlib.util.open_url(ref_path) as f:
        ref = dict(np.load(f))

    mu, sigma = just_calculate_inception_stats(image_path=image_path, num_expected=num_expected, seed=seed,
                                               max_batch_size=batch, make_patches=make_patches)

    fid = calculate_fid_from_inception_stats(mu, sigma, ref['mu'], ref['sigma'])
    return fid


def just_calculate_inception_stats(
    image_path, num_expected=None, seed=0, max_batch_size=64,
    num_workers=3, prefetch_factor=2, device=torch.device('cuda'),
    make_patches=((128, 128), (50, 50))
):
