...
class FeatureProvider:
    ...
    def init(self, dataset):
        ...
        self.cache_path = self.cache_dir / dataset.name / self.fe_tag / dataset.object_name
    ...
    def _save_on_disk(self):
        ...
        if self.fe is None:
            raise Exception(f"Path {str(self.cache_path)} does not exist and no Feature Extractor was loaded")
    ...
    def _get(self, ind):
        ...
        if self.save_on_disk:
            obj_name, class_name, img_name = self.path_tokens(img_path)
            file_path = self.cache_path / f"{class_name}_{img_name.split('.')[0]}.pt"
            return torch.load(file_path, map_location=self.device, weights_only=True)
        else:
            img = self.load_image(img_path)
            with torch.no_grad():
                return self.fe(img)[0]

class RGBFeatures(FeatureProvider):
    def __init__(self, cache_dir='cache', save_on_disk=True, save_in_memory=True, device='cpu'):
        super(RGBFeatures, self).__init__(cache_dir,
                                          save_on_disk=save_on_disk, save_in_memory=save_in_memory, device=device)
        self.fe_tag = "rgb"
        self.fe = torch.nn.Identity()
...
class FeaturesFromFolder(FeatureProvider):
    def __init__(self, fe_tag, cache_dir='cache', save_in_memory=True, device='cpu'):
